...
<?php
  ...
  if(isset($_POST['add_sale'])){
    $req_fields = array('s_id','sku','order_no','quantity','price','total', 'date' );
    validate_fields($req_fields);
        if(empty($errors)){
          $p_id      = $db->escape((int)$_POST['s_id']);
          $sku     = $db->escape($_POST['sku']);
          $order     = $db->escape($_POST['order_no']);
          ...
          $sql .= " product_id,order_no,qty,price,date";
          $sql .= ") VALUES (";
          $sql .= "'{$p_id}','{$order}','{$s_qty}','{$s_total}','{$s_date}'";
          $sql .= ")";
